  ...
  factory: (require) => {
    ...
    if (typeof document !== "undefined" && document.getElementById("dsh-md-annotator-style") === null) {
      ...
      styleEl.textContent = [
        ...
        ".mdan-panel{position:fixed;z-index:60;display:flex;flex-direction:column;background:var(--dsw-alias-bg-overlay);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;box-shadow:0 8px 24px rgba(0,0,0,.18);overflow:hidden}",
      ...
      function sendToDraft() {
        ...
        try {
          ...
          if (pluginSetting("clearAfterSend", true) === true) {
            persist([])
            setFeedback({ kind: "ok", text: "已写入对话框草稿并清空批注，回车即可发送" })
          } else {
            setFeedback({ kind: "ok", text: "已写入对话框草稿，回车即可发送（批注保留，可在设置中开启自动清空）" })
          }
        } catch (error) {
