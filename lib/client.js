...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Selection offsets are computed against an annotatable unit's rendered
    // textContent (the DOM text after markdown syntax is stripped), so they
    // stay stable across re-renders of the same block source.
    ...
    function boundaryOffset(root, node, offset) {
      var walker = document.createTreeWalker(root, NodeFilter.SHOW_ALL, null)
      var count = 0
      var n
    ...
    function rangeFromOffsets(root, start, end) {
      ...
      var count = 0
      var t, startNode = null, startOff = 0, endNode = null, endOff = 0
    ...
    function loadPanelPrefs(sessionId, path) {
      ...
      return prefs === undefined ? { open: false, barCollapsed: false } : prefs
    }
    ...
    function MdAnnotateView(props) {
      ...
      var selState = React.useState(null) // { ref, start, end, text, x, y } | null
      var sel = selState[0]
      var setSel = selState[1]
      var selectedState = React.useState([])
      var selectedIds = selectedState[0]
      var setSelectedIds = selectedState[1]
      ...
      function openEditorFor(note) {
        setEditing({ ref: note.ref, source: note.source, start: note.start, end: note.end, noteId: note.id, kind: typeof note.kind === "string" ? note.kind : lastKind, selStart: note.selStart, selEnd: note.selEnd, selText: note.selText })
      ...
      function captureSelection() {
        ...
        var unit = unitOf(range.commonAncestorContainer)
        if (unit === null) return null
        // both ends must sit inside the same annotatable unit (no cross-block)
        ...
        var start = boundaryOffset(unit, range.startContainer, range.startOffset)
        var end = boundaryOffset(unit, range.endContainer, range.endOffset)
        if (start === null || end === null) return null
        if (start > end) { var tmp = start; start = end; end = tmp }
        var text = unit.textContent.slice(start, end)
        if (text.trim() === "") return null
        return { ref: unit.getAttribute("data-mdan-ref"), start: start, end: end, text: text, rect: range.getBoundingClientRect() }
      }
      function handleSelection() {
        var pick = captureSelection()
        ...
        pick.y = pick.rect.bottom - sr.top + scrollEl.scrollTop + 6
        setSel(pick)
      }
      function openSelEditor() {
        if (sel === null) return
        var parts = sel.ref.split(":")
        var block = blocks[Number(parts[0])]
        if (block === undefined) { setSel(null); return }
        var item = block.type === "list" && parts.length > 1 ? block.items[Number(parts[1])] : undefined
        setEditing({
          ref: sel.ref,
      ...
      function saveEditing() {
        ...
        if (editing.noteId === undefined) {
          var note = {
            ...
            kind: kind,
            note: text,
            selStart: editing.selStart, selEnd: editing.selEnd, selText: editing.selText
          }
          persist(list.concat([note]))
        } else {
          persist(list.map(function (n) {
            return n.id === editing.noteId ? Object.assign({}, n, { note: text, kind: kind, selStart: editing.selStart, selEnd: editing.selEnd, selText: editing.selText }) : n
          }))
        }
        setEditing(null)
        setDraftText("")
      ...
      function togglePanel(next) {
        setPanel(function (prev) {
          ...
          // 面板可全视口浮动：钳制只保证标题栏抓取区留在视口内
          nextPrefs.x = clamp(nextPrefs.x, 8, Math.max(8, window.innerWidth - 160))
          nextPrefs.y = clamp(nextPrefs.y, 8, Math.max(8, window.innerHeight - 48))
          nextPrefs.w = clamp(typeof nextPrefs.w === "number" ? nextPrefs.w : 360, 240, Math.max(240, window.innerWidth - nextPrefs.x - 8))
          nextPrefs.h = clamp(typeof nextPrefs.h === "number" ? nextPrefs.h : 300, 160, Math.max(160, window.innerHeight - nextPrefs.y - 8))
      ...
      function toggleBar() {
        setPanel(function (prev) {
          var nextPrefs = Object.assign({}, prev, { barCollapsed: prev.barCollapsed !== true })
          savePanelPrefs(sessionId, path, nextPrefs)
          return nextPrefs
        })
      }
      ...
      function moveDrag(event) {
        ...
        if (d.kind === "move") {
          var x = clamp(d.baseX + dx, 8, Math.max(8, window.innerWidth - 160))
          var y = clamp(d.baseY + dy, 8, Math.max(8, window.innerHeight - 48))
          setPanel(function (p) { return Object.assign({}, p, { x: x, y: y }) })
        } else {
          var w = clamp(d.baseW + dx, 240, Math.max(240, window.innerWidth - (typeof panel.x === "number" ? panel.x : 8) - 8))
          var h = clamp(d.baseH + dy, 160, Math.max(160, window.innerHeight - (typeof panel.y === "number" ? panel.y : 8) - 8))
      ...
      function buildReport(notes) {
        var list = notes === undefined ? annotations : notes
        var out = []
        ...
        KIND_ORDER.forEach(function (kindId) {
          var group = list.filter(function (n) { return (typeof n.kind === "string" ? n.kind : "suggest") === kindId })
          ...
          group.forEach(function (n) {
            ...
            out.push(linesLabel)
            out.push("Excerpt:")
            excerptLines(n.selText !== undefined ? n.selText : n.source).forEach(function (l) { out.push("> " + l) })
      ...
      function sendNotes(notes) {
        ...
        try {
          ...
          var report = buildReport(notes)
          input.setDraft(current.trim() === "" ? report : current + "\n\n" + report)
          var sentIds = notes.map(function (n) { return n.id })
          if (pluginSetting("clearAfterSend", true) === true) {
            persist(loadAnnotations(sessionId, path).filter(function (n) { return sentIds.indexOf(n.id) === -1 }))
      ...
      function sendToDraft() {
        sendNotes(annotations)
      }
      function sendSelected() {
        sendNotes(annotations.filter(function (n) { return selectedIds.indexOf(n.id) !== -1 }))
      }
      ...
      function toggleSelectAll() {
        ...
        var allOn = annotations.every(function (n) { return selectedIds.indexOf(n.id) !== -1 })
      ...
      }, [editing, sel, panel.open, sessionId, path])

      // Paint text-selection annotation ranges through the CSS Custom
      // Highlight API (DOM untouched → no React conflicts). Degrades to
      // chips-only on engines without CSS.highlights.
      React.useEffect(function () {
        var canHighlight = typeof CSS !== "undefined" && CSS.highlights !== undefined && typeof Highlight === "function"
        if (!canHighlight) return undefined
        var ranges = []
        annotations.forEach(function (n) {
          if (n.selStart === undefined || n.selEnd === undefined || rootRef.current === null) return
          var el = rootRef.current.querySelector('[data-mdan-ref="' + n.ref + '"]')
          if (el === null) return
          var s = n.selStart
          var e = n.selEnd
          if (el.textContent.slice(s, e) !== n.selText) {
            var idx = el.textContent.indexOf(n.selText)
            if (idx === -1) return
            s = idx
            e = idx + n.selText.length
          }
          var r = rangeFromOffsets(el, s, e)
          if (r !== null) ranges.push(r)
        })
        if (ranges.length === 0) CSS.highlights.delete("mdan-sel")
        else CSS.highlights.set("mdan-sel", new (Function.prototype.bind.apply(Highlight, [null].concat(ranges)))())
      ...
      React.useEffect(function () {
        function onResize() {
          setPanel(function (p) {
            ...
            var x = clamp(p.x, 8, Math.max(8, vw - 160))
            var y = clamp(p.y, 8, Math.max(8, vh - 48))
            var next = Object.assign({}, p, {
              x: x, y: y,
              w: clamp(typeof p.w === "number" ? p.w : 360, 240, Math.max(240, vw - x - 8)),
              h: clamp(typeof p.h === "number" ? p.h : 300, 160, Math.max(160, vh - y - 8))
            })
            if (next.x === p.x && next.y === p.y && next.w === p.w && next.h === p.h) return p
            savePanelPrefs(sessionId, path, next)
            return next
          })
        }
      ...
      function editorPanel() {
        return React.createElement("div", { className: "mdan-editor" },
          editing !== null && editing.selText !== undefined ? React.createElement("div", { className: "mdan-editor-sel" }, "选中：" + truncate(editing.selText, 120)) : null,
      ...
      function renderListPanel() {
        var allOn = annotations.length > 0 && annotations.every(function (n) { return selectedIds.indexOf(n.id) !== -1 })
        var selectedCount = annotations.filter(function (n) { return selectedIds.indexOf(n.id) !== -1 }).length
        var rows = annotations.map(function (n, k) {
          var checked = selectedIds.indexOf(n.id) !== -1
          return React.createElement("div", { key: n.id, className: "mdan-row" },
            ...
            React.createElement("div", { className: "mdan-row-body" },
              ...
              React.createElement("div", { className: "mdan-row-quote" }, (n.selText !== undefined ? "选中：" + truncate(n.selText, 120) : "原文：" + truncate(n.source, 120)) + (isStale(n) ? "（原文已变化）" : ""))),
            React.createElement("button", {
              type: "button",
      ...
      function renderBar() {
        var collapsed = panel.barCollapsed === true
